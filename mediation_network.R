mediation_network <- function(d2f) {
  ...
  edge.width <- c("Low" =0.8,
                  ...
                  "Highest" = 2,
                  "None"=0,
                  "End" = 0.5) # "neutrale" verbindung zur basis
  E(d2.net)$width <- edge.width[as.character(d2.edges$beta.reduction)] + 0.6 #  + 0.6 # TAG TOPGENES
  ...
  # edge color
  edge.col <- c("Gene expression mediated metabolite effects" = pal.metabs[1],
                "Bi-directional mediation" = "palegreen3",
                "Metabolite mediated gene expression effects" = pal.genes[1],
                "Other" = "grey35",
                "End" = alpha("grey70",.5))
  ...
  # Legend: Edge Color
  legend(x="bottomleft",
         c("Gene expression mediated metabolite effect",
           "Bi-directional mediation effect",
           "Metabolite mediated gene expression effect",
           "Other",
           "No mediation tested"), 
         lty="solid",
         col=edge.col,
  ...
  legend(x="topright",
         ...
         lwd=c("Low" =0.6,
               "Middle"=1.1,
               "High"=1.6,
               "Highest"=2) +0.6, # + 0.6 # TAG TOPGENES
         bty="n",
         ncol=1,
         cex=lgd.cex
  )

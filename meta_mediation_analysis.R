meta_mediation_analysis <- function(
  phenotype,
  assocData
){
  
  ### Compute server --------------------------------------------------------------------------------
  computer <- "amanMRO" # specify exactly, i. e. with 'MRO' suffix (if applicable)
  
  ### Path for QC data ------------------------------------------------------------------------------
  pathProjectQC <- "/net/ifs1/san_projekte/projekte/genstat/02_projekte/1703_ge_metab_a1_b3_sorbs/1805_gxMetaboliteAssociation"
  ...
  # split into list
  assocData <- assocData[var %in% (phenotype), ]
  data <- lapply(unique(assocData$cohort), function(i){
    
    obj <- assocData[cohort == (i), .(markerID, x, m, x.type,id.number, codedAll, noncodedAll, eaf, imputed, infoscore, n, beta, se, p)]
    return(obj)
  })
  ...
  for (i in 1 : k) {
    ...
    # meta data specific to mediation
  ...
  metaData <- data.table(markerID = allMarkerID,
                         x = assocData[match(allMarkerID, assocData$markerID), x],
                         m = assocData[match(allMarkerID, assocData$markerID), m],
                         x.type = unique(assocData$x.type),
  ...
  for (i in 1 : k) {
    ...
    metaData <- data.table(metaData, 
                           signif(data[[i]][, beta], digits = signDig), 
                           signif(data[[i]][, se], digits = signDig),
                           signif(data[[i]][, p], digits = signDig), 
                           data[[i]][, n],
                           signif(data[[i]][, eaf],digits = signDig), 
                           signif(maf[, i], digits = signDig), 
                           imputed[, i], 
                           signif(infoscore[, i], digits = signDig))
  }

create_mediation_network_data <- function(
  ...
  #tests
  # stopifnot(d2[,diff.raw.vs.dir.gx == (r2.raw.gx - r2.dir.gx)] %>% all)
  # stopifnot(d2[,diff.raw.vs.dir.metab == (r2.raw.metab - r2.dir.metab)] %>% all)
  
  # annotate metabolite class
  d2[,m.class1:=res.init.meta[match(d2$metabolite,res.init.meta$metabolite),metab.class]]
  d2[,m.class2:=res.init.meta[match(d2$metabolite,res.init.meta$metabolite),metab.class.q]]
  
  # annotate super path
  d2[,m.path:=res.init.meta[match(d2$metabolite,res.init.meta$metabolite),metab.super.path]]
  d2[,g.path:=res.init.meta[match(d2$gene,res.init.meta$symbol_INGENUITY),types_INGENUITY]]
  
  # annotate proper metabolite name
  d2$metabolite.rid <- d2$metabolite
  d2$metabolite <- agn.all[match(d2$metabolite.rid, rid), abbr]
  ...
  d2[mediation.group=="Bi-directional mediation", 
     sign.beta.mediation :=   ifelse(sign(beta.mediation.gx) == sign(beta.mediation.metab),
                                     sign(beta.mediation.gx),
                                     "Both"),]
  d2[mediation.group=="Gene expression mediated metabolite effects", sign.beta.mediation := sign(beta.mediation.metab)]
  d2[mediation.group=="Metabolite mediated gene expression effects", sign.beta.mediation := sign(beta.mediation.gx)]
  d2[sign.beta.mediation == "-1", sign.beta.mediation := "Negative"]
  d2[sign.beta.mediation == "1", sign.beta.mediation := "Positive"]
  d2$sign.beta.mediation %>% mytable()
  
  # get beta direction of "Other" group
  d2[mediation.group=="Other" & ((sig.mediation.gx) == F | is.na(beta.mediation.gx)), sign.beta.mediation := sign(beta.mediation.metab)]
  d2[mediation.group=="Other" & ((sig.mediation.metab) == F | is.na(beta.mediation.metab)), sign.beta.mediation := sign(beta.mediation.gx)]
  d2[mediation.group=="Other" & !is.na(beta.mediation.metab) & !is.na(beta.mediation.gx) & 
  ...
  d2[mediation.group == "Other" & sign.beta.mediation == "-1", sign.beta.mediation := "Negative"]
  d2[mediation.group == "Other" & sign.beta.mediation == "1", sign.beta.mediation := "Positive"]
  d2[mediation.group=="Other" & !is.na(beta.mediation.metab) & !is.na(beta.mediation.gx) & 
  ...
  # check
  d2[mediation.group=="Other",.(sign.beta.mediation, 
  ...
  d2$cat.beta.reduction.gx %>% mytable
  ...
  d2$cat.beta.reduction.metab %>% mytable
       ...
       ifelse(
         ...
         cat.beta.reduction.metab, ifelse(
           ...
           cat.beta.reduction.gx, ifelse(
             mediation.group == "Bi-directional mediation" & quot.dir.vs.raw.gx < quot.dir.vs.raw.metab,
             cat.beta.reduction.gx, 
             ifelse(mediation.group == "Other" & mediation.direction == "Gx", cat.beta.reduction.gx,
                    ifelse(mediation.group == "Other" & mediation.direction == "Metab",cat.beta.reduction.metab,
                           ifelse(mediation.group == "Other" & mediation.direction == "Both" & (cat.beta.reduction.gx == cat.beta.reduction.metab),
                                  cat.beta.reduction.gx, "CHECK!"))))))]
  d2$beta.reduction %>% mytable()
  ...
  d2$r2.cat.gx %>% mytable
  d2$r2.cat.metab %>% mytable
